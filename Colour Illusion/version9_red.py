import numpy as np
import matplotlib.pyplot as plt
from matplotlib.path import Path
from matplotlib.patches import PathPatch
from shapely.geometry import Point, box

# ----- geometry -----------------------------------------------------

r = 1.0
top = (r/2, np.sqrt(3)*r/2)
bl  = (0, 0)
br  = (r, 0)

C = Point(top).buffer(r, 256)   # cyan
M = Point(bl).buffer(r, 256)    # magenta
Y = Point(br).buffer(r, 256)    # yellow

# subtractive colour regions
regions = [
    (C.difference(M.union(Y)), "cyan"),
    (M.difference(C.union(Y)), "magenta"),
    (Y.difference(C.union(M)), "yellow"),
    (C.intersection(M).difference(Y), "blue"),
    (C.intersection(Y).difference(M), "green"),
    (M.intersection(Y).difference(C), "red"),
    (C.intersection(M).intersection(Y), "black"),
]

# cyan-only region
C_only = C.difference(M.union(Y))

# ----- helper -------------------------------------------------------

def draw_geom(ax, geom, color):
    if geom.is_empty:
        return
    for g in getattr(geom, "geoms", [geom]):
        v = np.array(g.exterior.coords)
        codes = [Path.MOVETO] + [Path.LINETO]*(len(v)-2) + [Path.CLOSEPOLY]
        ax.add_patch(PathPatch(Path(v, codes), facecolor=color, edgecolor="none"))

# ----- plot ---------------------------------------------------------

fig, ax = plt.subplots()

# draw subtractive colour regions
for geom, col in regions:
    draw_geom(ax, geom, col)

xmin, xmax = -1.2, 2.2
ymin, ymax = -1.2, 2.2

stripe_w = 0.02
spacing  = 0.04

# ----- main vertical stripes ---------------------------------------

x = xmin
while x < xmax:
    stripe = box(x, ymin, x + stripe_w, ymax)

    # outside cyan → red stripes
    draw_geom(ax, stripe.difference(C), (1,0,0))

    # inside cyan → black stripes
    draw_geom(ax, stripe.intersection(C), "black")

    x += spacing

# ----- white stripes between them (cyan-only region) ---------------

x = xmin + spacing/2
while x < xmax:
    stripe = box(x, ymin, x + stripe_w, ymax)

    draw_geom(ax, stripe.intersection(C_only), "white")

    x += spacing

# ----- formatting ---------------------------------------------------

ax.set_aspect("equal")
ax.set_xlim(xmin, xmax)
ax.set_ylim(ymin, ymax)
ax.axis("off")

# ----- SVG output ---------------------------------------------------

plt.savefig("cmy_red_stripe_circles.svg", format="svg", bbox_inches="tight")
plt.show()