...
# draw subtractive colour regions
...
while x < xmax:
    ...
    # outside magenta → green stripes
    draw_geom(ax, stripe.difference(M), (0,1,0))

    # inside magenta → black stripes
    draw_geom(ax, stripe.intersection(M), "black")

    x += spacing

# ----- white stripes between them (magenta-only region) ------------

x = xmin + spacing/2
while x < xmax:
    stripe = box(x, ymin, x + stripe_w, ymax)

    draw_geom(ax, stripe.intersection(M_only), "white")
...
plt.savefig("cmy_green_stripe_circles.svg", format="svg", bbox_inches="tight")
plt.show()
