...
union = C.union(M).union(Y)

# border thickness
border_w = 0.08
border = union.buffer(border_w).difference(union)
...
# white stripes between them (yellow-only area)
x = xmin + spacing/2
while x < xmax:
    stripe = box(x, ymin, x + stripe_w, ymax)
...
plt.savefig("cmy_stripe_circles.svg", format="svg", bbox_inches="tight")
plt.show()